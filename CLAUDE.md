...
Primary navigation is capped at 7 items, single row, no wrapping: Home, Buy, Sell, Rent, Manage & Invest, About, Contact. Buy, Sell, Rent, Manage & Invest, and About use mega menus rather than long dropdown lists; Contact and Home are direct links. Full mega-menu contents, the utility navigation (Owner Portal, Tenant Portal, Client Login, etc. — never in the primary nav), the IA rules for landing/campaign pages, and the desktop/mobile UX requirements are documented in `docs/navigation-architecture.md`, which is canonical for navigation. This supersedes the flat 12-item nav originally listed here — no pages were dropped, only reorganized under the mega menus (see the mapping table in that doc). As of `docs/navigation-architecture.md` v1.2, Rentals is its own top-level item rather than nested under a generic "Services" label, and the standalone Resources mega menu folded into About — see that doc's revision history for why.
